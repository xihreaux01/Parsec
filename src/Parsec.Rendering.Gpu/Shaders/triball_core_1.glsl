// Parsec mined fold core: cand1 (ball-box)  (genome: ([('ball', 0.37, 1.32), ('box', 0.67)], 1.55))
...
float estimate(vec3 p) {
    ...
    for (int i = 0; i < fp.iterations; i++) {
        ...
        r2 = dot(z, z);                                       // ball fold 0.37/1.32
        if (r2 < 0.13690)      { t = 1.74240/0.13690; z *= t; dr *= t; }
        else if (r2 < 1.74240) { t = 1.74240/r2;        z *= t; dr *= t; }
        z = clamp(z, -0.6700, 0.6700) * 2.0 - z;   // box fold
        z = scale * z + c;                        // affine
        dr = dr * abs(scale) + 1.0;
